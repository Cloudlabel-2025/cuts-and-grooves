...
export default function ProcessNarrativeEditor({ page = 'process', section = 'narrative' }) {
    ...
    if (loading) return <div className="p-8 opacity-30 uppercase text-[10px] tracking-widest">Loading Narrative...</div>;

    return (
        <div className="space-y-12">
            <div className="flex justify-between items-end border-bottom border-gray-100 pb-8">
                <div>
                    <h2 className="text-3xl font-light uppercase tracking-tight">Narrative Section</h2>
                    <p className="text-[10px] uppercase tracking-widest text-gray-400 mt-2">Manage introductory text blocks</p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-12 max-w-4xl">
                <div className="space-y-4">
                    <label className="text-[10px] uppercase tracking-widest font-bold text-gray-400">Main Heading (Narrative)</label>
                    <textarea
                        className="w-full bg-gray-50 border border-gray-100 p-6 text-xl font-light focus:bg-white focus:border-black outline-none transition-all resize-none min-height-[120px]"
                        value={stagedContent.heading}
                        onChange={(e) => setStagedContent({ ...stagedContent, heading: e.target.value })}
                    />
                </div>
                <div className="space-y-4">
                    <label className="text-[10px] uppercase tracking-widest font-bold text-gray-400">Supporting Subtext</label>
                    <textarea
                        className="w-full bg-gray-50 border border-gray-100 p-6 text-sm leading-relaxed focus:bg-white focus:border-black outline-none transition-all resize-none min-height-[200px]"
                        value={stagedContent.subtext}
                        onChange={(e) => setStagedContent({ ...stagedContent, subtext: e.target.value })}
                    />
                </div>
            </div>
        </div>
